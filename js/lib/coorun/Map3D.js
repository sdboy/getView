...
var Map3D = CooMap.Class.extend({
  ...
  */
  ...
  "UpdateNodeActive": function (dynamicPathLayer) {
  ...
  //停止跟随模型或路径移动，漫游可用
  "CancelUpdateNodeActive": function (dynamicPathLayer) {
    var tlo = map.CreateLayerOptions("dynamicpathlayer");
    tlo.AddConfig("LayerOptionsName", "DynamicPathLayerOptions");
    tlo.AddConfig("NodeActive", "false");
    dynamicPathLayer.UpdateLayerOptions(tlo);
  },
  "CreateDraw2DPolyline":function(type){
    ...
    tlo.AddConfig("DrawType", type);  					//0 是矩形  1是圆   2是多边形   3是线
    tlo.AddConfig("LiftUp", "0"); 						//抬高高度
    var polyline = map.CreateLayer("AnalysisLayer", tlo); //创建分析图层，第一项参数必须为AnalysisLayer
    map.AddLayer(polyline); //添加分析图层
    return polyline;
  },
  "UpdateDraw2DPolyline":function(layer,points,color,width){
    ...
    mlo.AddConfig("LineWidth", width);  					//线宽
    mlo.AddConfig("Points", points); //更新绘制线的坐标，当更新点时必须设置IsLoad配置项为true
    mlo.AddConfig("DrawLineColor", color); 	//设置线的颜色（RGBA）
...
//		},
  ...
  "createTextLabelLayer":function(opt){
     ...
     * 其中priority由vp根据PagedNode结点的范围(minExtent, maxExtent)、
     * 其距离视点的距离、LOD层级mLODScale计算得到， 调度优先级越大，优先调度并显示 
     */
  ...
  /*获取播放路径关键点*/
  "CalcBuffer":function(buffer,pos,keyPos){
    /*jshint maxcomplexity:2 */
    // var buffer = 0.5;
    var mPos = pos.split(',');
  ...
  "createModelLabelLayer":function(shpUrl,modelUrl){
    ...
    var pSymbol = map.CreateSymbol("PointSymbol");
    // 点大小，范围0-10
    pSymbol.AddConfig("Size", "1");
    // 颜色值0-1（RGBA），最后一位代表透明度，0为透明，1为不透
    pSymbol.AddConfig("Color", "1.0,1.0,0.0,0");
    // 创建类型为ModelSymbol的符号，必须为ModelSymbol字符串
    var tSymbol = map.CreateSymbol("ModelSymbol");
    /*
      * 绕Z轴(世界坐标系方向相同 far->near)旋转,从far(-Z)向near(+Z)看去,逆时针为正，
      * 弧度表示,偏航角
      */
    tSymbol.AddConfig("Heading", "[Yaw]");
    /*
      * 绕Y轴(世界坐标系方向相同 down(-Y)向up(Y))旋转,从down(-Y)向up(+Y)看去,
      * 逆时针为正，弧度表示,滚转角
      */
    tSymbol.AddConfig("Pitch", "[Roll]");
    /*
      * 绕X轴(世界坐标系方向相同 left->right)旋转,从left(-X)向right(+X)看去,
      * 逆时针为正，弧度表示,俯仰角
      */
    tSymbol.AddConfig("Roll", "[Pitch]");
    // 模型x方向放大比例
    tSymbol.AddConfig("XScale", "[XScale]");
    // 模型y方向放大比例
    tSymbol.AddConfig("YScale", "[YScale]");
    // 模型z方向放大比例
    tSymbol.AddConfig("ZScale", "[ZScale]");
    // 是否开启接地线
    ...
    // 设置构建器符号为BuildGeometryFilter，必须为BuildGeometryFilter字符串
    // pStyle.AddFilterName("BuildGeometryFilter");
    // 将符号配置添加到该样式，第一参必须为ModelSymbol字符串
    pStyle.AddSymbol("ModelSymbol", tSymbol.GetConfig());
    /*
      * 设置构建器符号为SubstituteModelFilter，必须为SubstituteModelFilter字符串，
      * 此为图标符号化和模型符号化共有
      */
    ...
    tlo.AddConfig("FeatureSourceType", "ogr");
    // 创建矢量的属性字段，属性名：属性类型：类型长度：小数点后几位
    tlo.AddConfig("Fields", "Name:String:100:0,XScale:Float:100:4," +
      "YScale:Float:100:4,ZScale:Float:100:4,Yaw:Float:100:4," +
      "Roll:Float:100:4,Pitch:Float:100:4,lon:String:100:0," +
      "lat:String:100:0,height:String:100:0");
    /*
      * 几何类型 Point为点 Polyline为线 Polygon为面
      * 此项配置不能少或字符串一定不能错误，否则保存文件不成功
      */
  ...
  /**
   * <p>符号化模型添加</p>
   * @method addModelLabel
   * @author jg
   * @param { Object } layer 要素所在的图层
   * @param { Object } opt 数据对象，属性如下
   * @param { String } modelId 要素的属性
   * @param { Number } xScale X方向缩放比例
   * @param { Number } yScale Y方向缩放比例
   * @param { Number } zScale Z方向缩放比例
  ...
  "addModelLabel":function(layer,opt){
    ...
    var yaw = this.opt.yaw || 0;
    var roll = this.opt.roll || 0;
  ...
  /**
   * <p>更新模型标注</p>
   * @method updateModelLabel
   * @author jg
   * @param { Object } opt 数据对象，包括下面的属性
   * @param { Object } feature 待更新的要素对象
   * @param { Object } layer 要素所在的图层
   * @param { String } modelId 要素的属性
   * @param { Number } xScale X方向缩放比例
   * @param { Number } yScale Y方向缩放比例
  ...
  "updateModelLabel":function(opt){
    ...
    var Feature = this.opt.feature;
    var Layer = this.opt.layer;
    var ModelID = this.opt.modelId || 'default';
    var XScale = this.opt.xScale || 1;
    var YScale = this.opt.yScale || 1;
    var ZScale = this.opt.zScale || 1;
    var Yaw = this.opt.yaw || 0;
  ...
  "getModelValue":function(layer){
    // 此处的layer是创建标注拾取响应器返回的layer
      var content = layer.GetResponserResult();
    // 获取模型的id信息（人为自定义）
      var modelId = content.GetConfigValueByKey("name");
...
//        }             
      ...
      // 获取矢量图层要素最大ID
      // var featureId = exeditlineLayer.GetMaxFeatureID();
      // 设置FeatureID
      // addFeature.SetFeatureId(featureId + 1);             
   ...
   * @method addResAxis
   * @author jg
   * @param { Object } opt 数据对象
   * @param { Object } layer 要素所在图层
   * @param { String } pickColor 拾取颜色r g b a
   * @param { String } changeColor 是否变色，"ture"代表变色，"false"代表不变
   * @param { String } adjustment 微调部件步进值，默认为0.01
   * @param { String } dragSpeed 拖拽速度系数，默认为1
   * @return { Object } 响应器对象
   * @date 2018/4/12 11:41
   * @version V1.0.0
   */
  "addResAxis": function (opt) {
    this.opt = opt;
    var Layer = this.opt.layer;
    if(Layer === null || Layer === undefined) {
      return;
    }
    var PickColor = this.opt.pickColor || "1.0,1.0,0,0.01";
    var ChangeColor = this.opt.changeColor || "true";
    ...
    // 是否变色
    resp.AddConfig("IsChangeColor", ChangeColor);
    // x轴关联字段（红轴）
    resp.AddConfig("Heading", "Heading");
    // y轴关联字段（绿轴）
    resp.AddConfig("Pitch", "Pitch");
    // z轴关联字段（蓝轴）
    resp.AddConfig("Roll", "Roll");
    // 微调部件步进值，默认为0.01
    resp.AddConfig("Adjustment", Adjustment);
    // 拖拽速度系数，默认为1
    resp.AddConfig("DragSpeed", DragSpeed);
    // 坐标轴响应器，必须为AxisEditResponser
    var resCross = map.CreateResponser("AxisEditResponser", resp);
  ...
  /**
   * <p>移除辅助坐标系响应器</p>
   * @method removeResAxis
   * @author jg
   * @return { Null }
   * @date 2018/4/12 11:51
   * @version V1.0.0
   */
  "removeResAxis": function () {
    // 移除辅助坐标系响应器，必须为AxisEditResponser
   ...
   * @method getResult
   * @author jg
   * @return { Ojbect } resCross 要素属性集
   * @date 2018/4/12 12:01
   * @version V1.0.0
   */
  "getResult": function (resCross) {
    var result = {};
    // 获取绕x轴旋转弧度
    var xstr = resCross.GetResponserResult().GetConfigValueByKey("OXAngle");
    // 获取绕y轴旋转弧度
    var ystr = resCross.GetResponserResult().GetConfigValueByKey("OYAngle");
    // 获取绕z轴旋转弧度
    var zstr = resCross.GetResponserResult().GetConfigValueByKey("OZAngle");
    // 获取经纬度坐标值
    var pos = resCross.GetResponserResult().
      GetConfigValueByKey("LonLatPoint");
    result.xAngle = xstr;
    result.yAngle = ystr;
    result.zAngle = zstr;
    result.pos = pos;
   ...
   * @method createPlot
   * @author hqz
   * @param { String } value 标绘类型
   * @param { Object } opt 参数集合
   * @return { Object }
   * @version v6.0.8
   */
  "createPlot":function(value, opt){
    this.opt = opt;
    var  pointColor = this.opt.PointColor || "1,0,0,0.6";
    var  pointSize  =this.opt.PointSize || "6";
    var  lineColor =this.opt.LineColor || "0,1,0,0.5";
    var  lineWidth =this.opt.LineWidth || "10.0";
    var  lineStyle = this.opt.LineStyle || "0";
    var  planeColor = this.opt.PlaneColor || "1,1,0.5,0.5";
    var  liftUp = this.opt.LiftUp ||"10";
    var  blockHeight = this.opt.BlockHeight ||"0";
    var  stickType = this.opt.StickType || "0";
    var  points = this.opt.Points;
    // 创建分析图层配置，给配置起个名称，任意名称
    var mlo = map.CreateLayerOptions("plot");	
    // 创建配置类型, AnalysisLayerOptions代表分析图层数据配置，必须是此键值对
    mlo.AddConfig("LayerOptionsName", "AnalysisLayerOptions"); 
    // 数据源类型,代表缓冲区分析，必须是此键值对
    mlo.AddConfig("DataSourceTypeName", "as_plot"); 
    // 点颜色
    mlo.AddConfig("PointColor", pointColor);
    // 点大小 
    mlo.AddConfig("PointSize", pointSize);
    // 线框颜色
    mlo.AddConfig("LineColor", lineColor);
    // 线宽 
    mlo.AddConfig("LineWidth", lineWidth);	
    // 线样式 0 实线 ；1 虚线
    mlo.AddConfig("LineStyle", lineStyle);	
    // 面颜色
    mlo.AddConfig("PlaneColor", planeColor);
    // 抬高高度
    mlo.AddConfig("LiftUp", liftUp);
    // 体块高度默认0 不拉升
    mlo.AddConfig("BlockHeight",blockHeight);	
    // 贴地类型 默认0 ：0 不贴地 1 地形高度贴地 3 纹理贴地
    mlo.AddConfig("StickType",stickType);	
    // 标绘类型
    mlo.AddConfig("PlotType", value);	
    // MODE_EDIT-编辑模式	MODE_DRAW-绘制模式	MODE_NULL-默认配置
    mlo.AddConfig("PlotMode","MODE_DRAW");					
    if (points !== ""& points !== undefined){
      mlo.AddConfig("IsActive", "false");
      // MODE_EDIT-编辑模式	MODE_DRAW-绘制模式	MODE_NULL-默认配置
      mlo.AddConfig("PlotMode","MODE_NULL");
      // 点是否预先设置
      mlo.AddConfig("IsLoad","true");
      /*
       * "120.205420708,30.2416892474,15.0748782353;
       * 120.20489575,30.2417527978,-4.90313395858e-005;|" 
       * |表示单个图层中多个同类型点集分隔
       */
      mlo.AddConfig("Points",points); 
    }
    // 创建分析图层，第一项参数必须为AnalysisLayer
    var plot = map.CreateLayer("AnalysisLayer", mlo);
    // 添加分析图层
    map.AddLayer(plot); 									
    plot.AddObserver();
    return plot;
  ...
  "modifyPlot": function(layer, opt){	
    ...
    mlo.AddConfig("PointColor",pointColor);				//点颜色
    mlo.AddConfig("PointSize",pointSize);							//点大小
    mlo.AddConfig("LineColor", lineColor);				//线框颜色
    mlo.AddConfig("LineWidth", lineWidth);						//线宽
    mlo.AddConfig("LineStyle", lineStyle);						//线样式 0 实线 ；1 虚线
    mlo.AddConfig("PlaneColor", planeColor);				//面颜色
    mlo.AddConfig("LiftUp", liftUp);							//抬高高度
    mlo.AddConfig("BlockHeight",blockHeight);						//体块高度默认0 不拉升
    mlo.AddConfig("StickType",stickType);					//贴地类型 默认0 ：0 不贴地 1 地形高度贴地 3 纹理贴地
    //mlo.AddConfig("PlotType", value);						//标绘类型
    mlo.AddConfig("PlotMode","MODE_DRAW");					//MODE_EDIT-编辑模式	MODE_DRAW-绘制模式	MODE_NULL-默认配置
    layer.UpdateLayerOptions(mlo);							//更新标绘
    //return layer;
  },
  /**
   * 编辑标绘
   ...
   * @version v6.0.8
   */
  "editPlot":function(layer,opt){
    this.opt = opt;
    var  pointColor = this.opt.PointColor || "1,1,1,0.6";
    var  pointSize  =this.opt.PointSize || "5";
    var  lineColor =this.opt.LineColor || "0,0,0,0.5";
    var  lineWidth =this.opt.LineWidth || "10.0";
    var  lineStyle = this.opt.LineStyle || "0";
    var  planeColor = this.opt.PlaneColor || "1,1,0.5,0.5";
    var  liftUp = this.opt.LiftUp ||"30";
    var  blockHeight = this.opt.BlockHeight ||"0";
    var  stickType = this.opt.StickType || "0";
    /*
     * 编辑开始后，点击黄色的点可控制其位置；
     * 点击标绘表面，可整体拖拽标绘；右键为退出编辑模式
     */
    //创建分析图层配置，给配置起个名称，任意名称
    var mlo = map.CreateLayerOptions("plot");
    // 创建配置类型, AnalysisLayerOptions代表分析图层数据配置，必须是此键值对
    ...
    //	mlo.AddConfig("PlotType", value);
    // MODE_EDIT-编辑模式	MODE_DRAW-绘制模式	MODE_NULL-默认配置 
    mlo.AddConfig("PlotMode","MODE_EDIT");
    // 更新标绘			
    layer.UpdateLayerOptions(mlo);
  },
  /**
  ...
  "deletePlot": function(layer, opt){
    ...
    mlo.AddConfig("StickType",stickType);	
    /* 
     * MODE_EDIT-编辑模式	MODE_DRAW-绘制模式	MODE_DELETE-删除模式  
     * MODE_CLEAR-清除模式   MODE_NULL-默认配置
     */
    mlo.AddConfig("PlotMode","MODE_DELETE");							
    layer.UpdateLayerOptions(mlo);		
  }
});
